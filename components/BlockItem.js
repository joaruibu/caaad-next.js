...
const BlockItem = React.forwardRef(({ block, similarBlock }, ref) => {
  ...
  return (
    <article
      key={_id}
      className={` ${
        isSimilarBlock ? " w-56 flex-none" : " w-full "
      } border border-orange-600  rounded-3xl overflow-hidden sm:hover:shadow-[5px_7px_0px_0px_rgb(234,88,12)] transition-all `}
    >
      <Link
        className="cursorHover cursor-pointer"
        href={`/download-autocad-block/${urlTitle(
        ...
        <Image
          ...
          alt={
            locale === "es"
              ...
              : `Dowload free ${title} autocad block. Autocad block make by Be Interior Designer for block free download`
          }
          src={img}
          loading="lazy"
          unoptimized={true}
        ...
        <div className="flex flex-wrap gap-1 p-3">
          {
            <>
              {filters.map((filter, index) => {
                return <BadgeFilter key={index} filter={filter} icon={false} />;
              })}
              {tags
